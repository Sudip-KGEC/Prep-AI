...
export default function EarningsSection({ stats, history }) {
  ...
  const { data, loading, error, fn: withdrawFn } =
    useFetch(requestWithdrawal);
    ...
    <section className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div
            key={stat.label}
            className="bg-[#0f0f11] border border-purple-500/10 rounded-2xl p-6 flex flex-col gap-2 shadow-[0_0_30px_rgba(168,85,247,0.04)]"
          >
      ...
      {/* Withdraw card */}
      <div className="bg-[#0f0f11] border border-purple-500/10 rounded-2xl p-8 flex items-center justify-between gap-4 shadow-[0_0_40px_rgba(168,85,247,0.05)]">
        <div>
          ...
          <p className="text-xs text-stone-500 font-light mt-1">
            20% platform fee applies. Processed within 2–3 business
            days.
          </p>
        </div>
      ...
      {/* History */}
      {history?.length > 0 && (
        <div className="bg-[#0f0f11] border border-purple-500/10 rounded-2xl p-8 flex flex-col gap-5 shadow-[0_0_40px_rgba(168,85,247,0.04)]">
          ...
          <div className="flex flex-col gap-3">
            {history.map((p) => (
              <div
                key={p.id}
                className="flex items-center justify-between bg-[#141417] border border-purple-500/10 rounded-xl px-5 py-4"
              >
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="bg-[#0f0f11] border border-purple-500/10 text-stone-100 max-w-md shadow-[0_0_50px_rgba(168,85,247,0.08)]">
          {done ? (
            <div className="py-8 text-center flex flex-col items-center gap-4">
              <span className="w-14 h-14 rounded-full bg-purple-500/10 border border-purple-500/20 flex items-center justify-center text-2xl text-purple-400">
                ✓
              </span>
              ...
              <p className="text-xs text-stone-500 font-light">
                We&apos;ll process your withdrawal within 2–3 business
                days.
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="flex flex-col gap-5 py-2">
                {/* Fee */}
                <div className="rounded-xl bg-[#141417] border border-purple-500/10 p-4 flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  >
                    <TabsList className="bg-[#141417] border border-purple-500/10 w-full">
                      {PAYMENT_METHODS.map((m) => (
                        <TabsTrigger
                          key={m.value}
                          value={m.value}
                          className="flex-1 text-xs data-[state=active]:bg-purple-500/10 data-[state=active]:text-purple-400"
                        >
                          {m.label}
                        </TabsTrigger>
                      ))}
