        ...
        return (
          <div
            ...
            className={`relative rounded-2xl p-10 h-full flex flex-col transition-all duration-300 hover:-translate-y-1 ${
              plan.featured
                ? "bg-[#141417] border border-purple-400/20"
                : "bg-[#0f0f11] border border-white/10 hover:border-purple-400/10"
            } ${isActive ? "ring-1 ring-purple-400/30" : ""}`}
          >
            {/* Most Popular badge */}
            {plan.featured && !isActive && (
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-purple-400 text-[#0a0a0b] text-xs font-bold tracking-wide uppercase px-3.5 py-1 rounded-full whitespace-nowrap">
                Most Popular
              </span>
            )}
            ...
            <div className="flex items-end gap-1 mb-1.5">
              <span
                className={`font-serif text-5xl leading-none tracking-tight ${
                  plan.featured
                    ? "bg-linear-to-br from-purple-400 to-purple-600 bg-clip-text text-transparent"
            ...
            <p className="text-sm text-purple-400 mb-7">{plan.credits}</p>
